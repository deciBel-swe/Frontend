...
import { Play, Heart, MoreHorizontal, MessageCircle, Repeat } from 'lucide-react';
    ...
    <div className="group flex w-full items-center gap-3 px-2 py-3 rounded-xl transition hover:bg-surface-raised">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex items-center gap-3 text-xs mt-1">
          ...
          <Link href={statsLinks.likes} className="flex items-center gap-1 hover:text-status-error transition">
            <Heart size={14} /> {stats.likes}
          </Link>

          <Link href={statsLinks.reposts} className="flex items-center gap-1 hover:text-status-success transition">
            <Repeat size={14} /> {stats.reposts}
          </Link>

          <Link href={statsLinks.comments} className="flex items-center gap-1 hover:text-status-info transition">
