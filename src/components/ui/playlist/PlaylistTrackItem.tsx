    ...
    >
      {[0, 150, 300].map((delay) => (
        <span
          ...
          style={{
            animation: `playingBar 0.6s ease ${delay}ms infinite alternate`,
          }}
        />
      ))}
      <style>{`
...
type PlaylistTrackItemProps = {
  ...
  // New props for Drag & Drop and Removal
  isOwner?: boolean;
  draggable?: boolean;
  ...
  return (
    <li
      ...
      className={`
        ...
        ${isDragTarget ? 'ring-1 ring-brand-primary/60 bg-surface-raised' : 'ring-0'}
      `}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      onClick={onPlay}
    >
      {/* Drag Handle (Visible for Owner on Hover) */}
      {isOwner && (
        <div className="w-4 flex items-center justify-center text-text-secondary cursor-grab active:cursor-grabbing">
          <GripVertical
            size={16}
            className={`transition-opacity ${hovered || isDragging ? 'opacity-60' : 'opacity-0'}`}
          />
        </div>
      )}
      {/* Cover */}
      ...
      {/* Number */}
      <span
        className={`text-sm font-bold w-5 text-right shrink-0 ${isActive ? 'text-brand-primary' : 'text-text-muted'}`}
      >
        {index + 1}.
      </span>
