...
export default function Timeline({ duration, currentTime, onScrub }: TimelineProps) {
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-2xl group select-none">
      {/* Current Time */}
      <span className="text-xs font-bold text-text-primary tabular-nums min-w-[22px]">
        {format(currentTime)}
      </span>
      ...
      >
        ...
        <div 
          className="absolute w-3 h-3 bg-surface-raised border-2 border-brand-primary rounded-full shadow-lg transition-opacity duration-150 opacity-0 group-hover:opacity-100"
          style={{ 
            left: `${progress}%`,
      ...
      {/* Duration */}
      <span className="text-xs font-bold text-text-primary tabular-nums min-w-[22px] text-right">
        {format(duration)}
      </span>
    </div>
  );
