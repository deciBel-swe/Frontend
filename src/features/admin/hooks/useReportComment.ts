...
export function useReportComment() {
  const [reportCommentResponse, setReportCommentResponse] =
  ...
  const reportComment = useCallback(
    async (commentId: number, payload: ReportRequest) => {
      setIsLoading(true);
      setIsError(false);
      setError(null);

      try {
        const result = await adminService.reportComment(commentId, payload);
        setReportCommentResponse(result);
        return result;
      } catch (caughtError) {
